...
import (
	"api-order/src/data/airquality/application" // Ajustado
...
// @Security     BearerAuth
// @Success      200  {object}  responses.Response{data=[]entities.AirQualityData} "airquality records retrieved successfully"
...
// @Failure      500  {object}  responses.Response "Internal server error while retrieving airquality records"
// @Router       /data/airquality/kit/{kit_id}/minutes/{minutes} [get]
func (ctr *GetMinutesRecordsController) Run(ctx *gin.Context) {
	...
	if err != nil {
		log.Printf("Error getting air quality records for kit %d (%d min): %v", kitID, minutes, err) // Ajustado log
		ctx.JSON(http.StatusInternalServerError, responses.Response{
			Success: false, Message: "Failed to retrieve air quality records.", Error: "An internal error occurred.", Data: nil, // Ajustado mensaje
		})
		return
	}
	...
	ctx.JSON(http.StatusOK, responses.Response{
		Success: true, Message: "Air quality records retrieved successfully.", Data: records, Error: nil, // Ajustado mensaje
	})
}
