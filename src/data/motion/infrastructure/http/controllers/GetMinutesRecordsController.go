...
import (
	"api-order/src/data/motion/application" // Ajustado
...
func (ctr *GetMinutesRecordsController) Run(ctx *gin.Context) {
	...
	if err != nil {
		log.Printf("Error getting motion records for kit %d (%d min): %v", kitID, minutes, err) // Ajustado log
		ctx.JSON(http.StatusInternalServerError, responses.Response{
			Success: false, Message: "Failed to retrieve motion records.", Error: "An internal error occurred.", Data: nil, // Ajustado mensaje
		})
		return
	}
	...
	ctx.JSON(http.StatusOK, responses.Response{
		Success: true, Message: "Motion records retrieved successfully.", Data: records, Error: nil, // Ajustado mensaje
	})
}
